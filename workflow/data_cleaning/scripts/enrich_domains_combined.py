...
def generate_domains_summary_stats(enriched_domains):
    ...
    if "political_leaning_score" in enriched_domains.columns:
        ...
        pol_citations_covered = enriched_domains[enriched_domains["political_leaning_score"].notna()]["citation_count"].sum()
        
        print(f"\nPolitical leaning coverage:")
        print(f"  Domains: {pol_domains_covered:,} / {total_domains:,} ({pol_domains_covered / total_domains * 100:.1f}%)")
        print(f"  Citations: {pol_citations_covered:,} / {total_citations:,} ({pol_citations_covered / total_citations * 100:.1f}%)")
        ...
        if "political_leaning" in enriched_domains.columns:
            ...
            for category in ["left_leaning", "right_leaning", "unknown"]:
                cat_citations = enriched_domains[enriched_domains["political_leaning"] == category]["citation_count"].sum()
                cat_pct = cat_citations / total_citations * 100
                cat_domains = (enriched_domains["political_leaning"] == category).sum()
                print(f"    {category}: {cat_citations:,} citations ({cat_pct:.1f}%) from {cat_domains:,} domains")
    ...
    if "domain_quality_score" in enriched_domains.columns:
        ...
        qual_citations_covered = enriched_domains[enriched_domains["domain_quality_score"].notna()]["citation_count"].sum()
        
        print(f"\nDomain quality coverage:")
        print(f"  Domains: {qual_domains_covered:,} / {total_domains:,} ({qual_domains_covered / total_domains * 100:.1f}%)")
        print(f"  Citations: {qual_citations_covered:,} / {total_citations:,} ({qual_citations_covered / total_citations * 100:.1f}%)")
        ...
        if "domain_quality" in enriched_domains.columns:
            ...
            for category in ["high_quality", "low_quality", "unknown"]:
                cat_citations = enriched_domains[enriched_domains["domain_quality"] == category]["citation_count"].sum()
                cat_pct = cat_citations / total_citations * 100
                cat_domains = (enriched_domains["domain_quality"] == category).sum()
                print(f"    {category}: {cat_citations:,} citations ({cat_pct:.1f}%) from {cat_domains:,} domains")
    
    # Combined coverage
    if ("political_leaning_score" in enriched_domains.columns and 
        "domain_quality_score" in enriched_domains.columns):
        combined_domains = ((enriched_domains["political_leaning_score"].notna()) &
                           (enriched_domains["domain_quality_score"].notna())).sum()
        ...
        print(f"  Domains: {combined_domains:,} / {total_domains:,} ({combined_domains / total_domains * 100:.1f}%)")
        print(f"  Citations: {combined_citations:,} / {total_citations:,} ({combined_citations / total_citations * 100:.1f}%)")
    ...
    for _, row in top_domains.iterrows():
        ...
        signals = []
        if "political_leaning" in row and pd.notna(row['political_leaning']) and row['political_leaning'] != 'unknown':
            signals.append(f"pol:{row['political_leaning']}")
        if "domain_quality" in row and pd.notna(row['domain_quality']) and row['domain_quality'] != 'unknown':
...
def main():
    ...
    print(f"Loaded {len(domains):,} unique domains representing {domains['citation_count'].sum():,} citations")
