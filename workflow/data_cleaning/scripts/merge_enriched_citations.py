...
def merge_enriched_domains_to_citations(citations_df, enriched_domains_df):
    ...
    enriched_citations = citations_df.merge(domain_signals, on="domain", how="left")

    print(
        f"Merge completed: {len(enriched_citations):,} citations with {len(enriched_citations.columns)} columns"
    )

    # Verify no citations were lost
    if len(enriched_citations) != len(citations_df):
        print(
            f"⚠️  Warning: Citation count changed from {len(citations_df)} to {len(enriched_citations)}"
        )
    else:
...
def generate_final_summary_stats(enriched_citations):
    ...
    if "political_leaning_score" in enriched_citations.columns:
        ...
        pol_domains_covered = enriched_citations[
    ...
    if "domain_quality_score" in enriched_citations.columns:
        ...
        qual_domains_covered = enriched_citations[
    ...
    # Combined coverage
    if (
        "political_leaning_score" in enriched_citations.columns
        and "domain_quality_score" in enriched_citations.columns
    ):
        combined_coverage = (
            (enriched_citations["political_leaning_score"].notna())
            & (enriched_citations["domain_quality_score"].notna())
...
def main():
    ...
    # Merge enriched domains back to citations
    enriched_citations = merge_enriched_domains_to_citations(
        citations, enriched_domains
    )
