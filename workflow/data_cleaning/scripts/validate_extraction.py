...
def validate_extraction():
    ...
    try:
        ...
        enriched_citations_file = citations_file.replace(
            "citations.parquet", "citations_enriched.parquet"
        )
        try:
    ...
    log_result(
        "Threads table not empty",
        len(threads_df) > 0,
        f"Expected >0, got {len(threads_df)}",
    )
    log_result(
        "Questions table not empty",
        len(questions_df) > 0,
        f"Expected >0, got {len(questions_df)}",
    )
    log_result(
        "Responses table not empty",
        len(responses_df) > 0,
        f"Expected >0, got {len(responses_df)}",
    )
    log_result(
        "Citations table not empty",
        len(citations_df) > 0,
    ...
    log_result(
        "Threads count matches expected",
        len(threads_df) == expected_threads,
    ...
    log_result(
        "Responses count is 2x questions",
        len(responses_df) == expected_responses,
    ...
    log_result(
        "All question thread_ids reference valid threads",
    ...
    log_result(
        "All response question_ids reference valid questions",
    ...
    log_result(
        "All citation response_ids reference valid responses",
    ...
    log_result(
        "Model sides are balanced",
        abs(side_a_count - side_b_count) <= 10,
    ...
    log_result(
        "Turn numbers consistent with total_turns",
        turn_mismatches == 0,
        f"Found {turn_mismatches} threads with inconsistent turn counts",
        warning=turn_mismatches < len(threads_df) * 0.1,
    )  # Warning if <10%
    ...
    for table_name, required_cols in required_fields.items():
        ...
        for col in required_cols:
            if col in df.columns:
                ...
                log_result(
                    f"{table_name}.{col} has no nulls",
                    null_count == 0,
                    f"Found {null_count} null values ({null_count / len(df) * 100:.1f}%)",
                    warning=null_count < len(df) * 0.05,
                )  # Warning if <5%
            else:
                log_result(
                    f"{table_name}.{col} column exists",
                    False,
    ...
    if "domain" in citations_df.columns:
        ...
        domain_completeness = (
            (len(citations_df) - domain_null_count) / len(citations_df) * 100
        )

        log_result(
            "High domain extraction completeness",
            domain_completeness >= 95.0,
    ...
    if enriched_citations_df is not None:
        ...
        log_result(
            "Enriched citations row count matches original",
        ...
        if "political_leaning_score" in enriched_citations_df.columns:
            pol_coverage = (
                enriched_citations_df["political_leaning_score"].notna().sum()
            )
            pol_pct = pol_coverage / enriched_count * 100
            log_result(
                "Political leaning data coverage",
                pol_pct >= 30.0,
            ...
            if "political_leaning" in enriched_citations_df.columns:
                leaning_counts = enriched_citations_df[
                    "political_leaning"
                ].value_counts()
                ...
                log_result(
                    "Categorical leaning variable created",
                    "political_leaning" in enriched_citations_df.columns,
        ...
        if "domain_quality_score" in enriched_citations_df.columns:
            ...
            if "domain_quality" in enriched_citations_df.columns:
                ...
                log_result(
                    "Categorical quality variable created",
                    "domain_quality" in enriched_citations_df.columns,
                    f"High: {high_count:,}, Low: {low_count:,}, Unknown: {unknown_count:,}",
                )

        # Check for combined coverage
        if (
            "political_leaning_score" in enriched_citations_df.columns
            and "domain_quality_score" in enriched_citations_df.columns
        ):
            combined_coverage = (
                (enriched_citations_df["political_leaning_score"].notna())
                & (enriched_citations_df["domain_quality_score"].notna())
            ).sum()
            combined_pct = combined_coverage / enriched_count * 100
            log_result(
                "Combined metrics coverage",
                combined_pct >= 15.0,
        ...
        if "political_leaning_score" in enriched_citations_df.columns:
            ...
            enrichment_stats.append(
                f"Political leaning range: {pol_stats['min']:.3f} to {pol_stats['max']:.3f}"
            )

        if "domain_quality_score" in enriched_citations_df.columns:
            qual_stats = enriched_citations_df["domain_quality_score"].describe()
            enrichment_stats.append(
                f"Domain quality range: {qual_stats['min']:.3f} to {qual_stats['max']:.3f}"
            )
    ...
    if validation_results["failed"] == 0:
        ...
        overall_status = (
            f"✗ ISSUES FOUND - {validation_results['failed']} validations failed"
        )
