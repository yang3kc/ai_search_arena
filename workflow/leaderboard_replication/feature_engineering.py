...
def create_conversation_metadata(
    ...
    full_response_data = response_metrics_df.merge(
        citation_metrics_df, on="response_id", how="left"
    ).fillna(0)
    ...
    for thread_id, thread_responses in full_response_data.groupby("thread_id"):
        ...
        if len(side_a_responses) > 0:
            ...
            conv_data["standardized_citations_a"] = side_a_responses[
                "standardized_citations"
            ].iloc[0]
            ...
            for category in [
                ...
                conv_data[f"cites_{category}_a"] = side_a_responses[
                    f"cites_{category}"
                ].max()
                conv_data[f"num_cites_{category}_a"] = side_a_responses[
                    f"num_cites_{category}"
                ].sum()
        ...
        if len(side_b_responses) > 0:
            ...
            conv_data["standardized_citations_b"] = side_b_responses[
                "standardized_citations"
            ].iloc[0]
            ...
            for category in [
                ...
                conv_data[f"cites_{category}_b"] = side_b_responses[
                    f"cites_{category}"
                ].max()
                conv_data[f"num_cites_{category}_b"] = side_b_responses[
                    f"num_cites_{category}"
                ].sum()
...
def create_leaderboard_format(
    threads_df: pd.DataFrame, responses_df: pd.DataFrame, citations_df: pd.DataFrame
) -> pd.DataFrame:
    """
...
def test_feature_engineering():
    ...
    print(
        "Sample conv_metadata keys:",
        list(leaderboard_df["conv_metadata"].iloc[0].keys())[:10],
    )

    # Check if we have the expected structure
    sample_conv = leaderboard_df.iloc[0]
    print(
        f"Sample conversation: {sample_conv['model_a']} vs {sample_conv['model_b']} -> {sample_conv['winner']}"
    )
