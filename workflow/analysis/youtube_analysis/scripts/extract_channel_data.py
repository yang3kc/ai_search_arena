...
def extract_unique_channels(video_metadata_df):
    ...
    channel_stats = (
        video_metadata_df.groupby(["channel_id", "channel_title"])
        .agg(
            {
    ...
    channel_stats.columns = [
        ...
        "channel_title",
        "cited_video_count",
        "total_views_of_cited_videos",
        "avg_views_of_cited_videos",
        "max_views_of_cited_videos",
        "earliest_cited_video",
        "latest_cited_video",
    ]
...
def generate_channel_summary(channel_stats):
    ...
    print(
        f"  Channels with 1 cited video: {(channel_stats['cited_video_count'] == 1).sum():,}"
    )
    print(
        f"  Channels with 2-5 cited videos: {((channel_stats['cited_video_count'] >= 2) & (channel_stats['cited_video_count'] <= 5)).sum():,}"
    )
    print(
        f"  Channels with 6-10 cited videos: {((channel_stats['cited_video_count'] >= 6) & (channel_stats['cited_video_count'] <= 10)).sum():,}"
    )
    print(
        f"  Channels with 11+ cited videos: {(channel_stats['cited_video_count'] >= 11).sum():,}"
    )
    ...
    for _, row in top_channels.iterrows():
        channel_name = (
            row["channel_title"] if row["channel_title"] else "Unknown Channel"
        )
        print(f"  {channel_name}: {row['cited_video_count']} videos")
        print(f"    Channel ID: {row['channel_id']}")
        print(
            f"    Total views of cited videos: {row['total_views_of_cited_videos']:,.0f}"
        )
        print()
    ...
    if (channel_stats["total_views_of_cited_videos"] > 0).any():
        ...
        print(
            f"  Total views across all cited videos: {view_stats['total_views_of_cited_videos'].sum():,.0f}"
        )
        print(
            f"  Average total views per channel: {view_stats['total_views_of_cited_videos'].mean():,.0f}"
        )
        print(
            f"  Median total views per channel: {view_stats['total_views_of_cited_videos'].median():,.0f}"
        )
    ...
    print("\nChannel name availability:")
    print(
        f"  Channels with names: {named_channels:,} ({named_channels / total_channels * 100:.1f}%)"
    )
    print(
        f"  Channels without names: {unnamed_channels:,} ({unnamed_channels / total_channels * 100:.1f}%)"
    )
