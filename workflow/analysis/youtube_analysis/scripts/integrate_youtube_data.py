...
def integrate_youtube_datasets(youtube_citations, video_metadata, channel_metadata):
    ...
    print(f"Video metadata coverage: {video_coverage:,}/{len(integrated_data):,} ({video_coverage/len(integrated_data)*100:.1f}%)")
    ...
    print(f"Channel metadata coverage: {channel_coverage:,}/{len(integrated_data):,} ({channel_coverage/len(integrated_data)*100:.1f}%)")
...
def clean_and_optimize_dataset(integrated_data):
    ...
    for col in integrated_data.columns:
        if col.endswith('_video_meta') or col.endswith('_channel_meta'):
            ...
            if base_col in integrated_data.columns:
                ...
                integrated_data[base_col] = integrated_data[col].fillna(integrated_data[base_col])
    ...
    numeric_columns = [
        ...
        'subscriber_count', 'video_count', 'citation_number', 'citation_order'
    ]
    for col in numeric_columns:
        if col in integrated_data.columns:
            integrated_data[col] = pd.to_numeric(integrated_data[col], errors='coerce').fillna(0).astype('int64')
    
    # Convert boolean columns
    boolean_columns = ['captions_available', 'hidden_subscriber_count', 'is_linked', 'made_for_kids']
    ...
    print(f"Memory usage: {integrated_data.memory_usage(deep=True).sum() / 1024**2:.1f} MB")
...
def main():
    ...
    integrated_data = integrate_youtube_datasets(youtube_citations, video_metadata, channel_metadata)
