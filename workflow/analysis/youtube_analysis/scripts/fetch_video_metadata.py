...
def fetch_all_video_metadata(youtube_citations):
    ...
    # Process in batches
    for batch_num, video_ids_batch in enumerate(batch_video_ids(video_ids, batch_size), 1):
        print(f"Processing batch {batch_num}/{(len(video_ids) + batch_size - 1) // batch_size} "
        ...
        if snakemake.config.get('preserve_raw_responses', True):
            save_raw_responses(success_videos, snakemake.output.api_responses_dir, batch_num)
...
def generate_video_metadata_summary(video_metadata_df):
    ...
    print(f"  Videos with descriptions: {video_metadata_df['description'].notna().sum():,}")
    ...
    if (video_metadata_df['view_count'] > 0).any():
        print(f"\nView count statistics:")
        view_stats = video_metadata_df[video_metadata_df['view_count'] > 0]['view_count']
    ...
    if 'published_at' in video_metadata_df.columns:
        video_metadata_df['published_year'] = pd.to_datetime(video_metadata_df['published_at']).dt.year
...
def main():
    ...
    video_metadata_list, failed_video_ids = fetch_all_video_metadata(youtube_citations)
    ...
    if video_metadata_list:
        ...
        empty_df = pd.DataFrame(columns=[
            ...
            'published_at', 'category_id', 'default_language', 'tags',
            'view_count', 'like_count', 'comment_count', 'duration',
            'captions_available', 'privacy_status', 'upload_status', 'api_fetch_date'
        ])
