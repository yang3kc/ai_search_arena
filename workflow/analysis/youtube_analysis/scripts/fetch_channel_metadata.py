...
def fetch_all_channel_metadata(unique_channels_df):
    ...
    # Process in batches
    for batch_num, channel_ids_batch in enumerate(
        batch_channel_ids(channel_ids, batch_size), 1
    ):
        print(
            f"Processing batch {batch_num}/{(len(channel_ids) + batch_size - 1) // batch_size} "
        ...
        if snakemake.config.get("preserve_raw_responses", True):
            save_raw_responses(
                success_channels, snakemake.output.api_responses_dir, batch_num
            )
...
def generate_channel_metadata_summary(channel_metadata_df):
    ...
    print(
        f"  Channels with descriptions: {channel_metadata_df['description'].notna().sum():,}"
    )
    print(
        f"  Channels with custom URLs: {channel_metadata_df['custom_url'].notna().sum():,}"
    )
    print(
        f"  Channels with country info: {channel_metadata_df['country'].notna().sum():,}"
    )

    # Subscriber statistics
    if (channel_metadata_df["subscriber_count"] > 0).any():
        print("\nSubscriber statistics:")
        sub_stats = channel_metadata_df[channel_metadata_df["subscriber_count"] > 0][
            "subscriber_count"
        ]
        ...
        print(
            f"  1K-10K subscribers: {((sub_stats >= 1000) & (sub_stats < 10000)).sum():,}"
        )
        print(
            f"  10K-100K subscribers: {((sub_stats >= 10000) & (sub_stats < 100000)).sum():,}"
        )
        print(
            f"  100K-1M subscribers: {((sub_stats >= 100000) & (sub_stats < 1000000)).sum():,}"
        )
    ...
    if (channel_metadata_df["video_count"] > 0).any():
        print("\nVideo count statistics:")
        video_stats = channel_metadata_df[channel_metadata_df["video_count"] > 0][
            "video_count"
        ]
    ...
    if (channel_metadata_df["view_count"] > 0).any():
        ...
        view_stats = channel_metadata_df[channel_metadata_df["view_count"] > 0][
            "view_count"
        ]
    ...
    if "published_at" in channel_metadata_df.columns:
        channel_metadata_df["created_year"] = pd.to_datetime(
            channel_metadata_df["published_at"], errors="coerce"
        ).dt.year
...
def main():
    ...
    channel_metadata_list, failed_channel_ids = fetch_all_channel_metadata(
        unique_channels
    )
