...
def extract_youtube_citations(citations_df):
    ...
    youtube_citations['video_id'] = youtube_citations['url'].apply(extract_video_id_from_url)
    ...
    # Remove duplicates based on video_id, keeping first occurrence
    before_dedup = len(youtube_citations)
    youtube_citations = youtube_citations.drop_duplicates(subset=['video_id'], keep='first')
    
    print(f"After deduplication: {len(youtube_citations):,} unique videos")
    print(f"Removed {before_dedup - len(youtube_citations):,} duplicate video citations")
    ...
    if invalid_count > 0:
        ...
        print("Sample invalid IDs:", youtube_citations[~valid_mask]['video_id'].head(3).tolist())
...
def generate_summary_stats(youtube_citations):
    ...
    for video_id, count in top_videos.items():
        sample_url = youtube_citations[youtube_citations['video_id'] == video_id]['url'].iloc[0]
        print(f"  {video_id}: {count} citations - {sample_url}")
    
    # URL format distribution
    print(f"\nURL format distribution:")
    url_formats = {
        'youtube.com/watch': youtube_citations['url'].str.contains('youtube.com/watch', case=False).sum(),
        'youtu.be': youtube_citations['url'].str.contains('youtu.be/', case=False).sum(),
        'youtube.com/embed': youtube_citations['url'].str.contains('youtube.com/embed', case=False).sum(),
        'm.youtube.com': youtube_citations['url'].str.contains('m.youtube.com', case=False).sum()
    }
