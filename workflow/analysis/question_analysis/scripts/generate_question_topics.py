...
def load_question_data(questions_path, embeddings_path):
    ...
    question_data = embeddings_df.merge(
        questions_df[['question_id', 'user_query']], 
        on='question_id'
    )
...
def extract_embeddings_and_queries(question_data):
    ...
    embedding_cols = [col for col in question_data.columns if col.startswith("embedding_dim_")]
    ...
    logger.info(f"Extracted {len(queries):,} queries with {len(embedding_cols)} embedding dimensions")
...
def generate_topic_distributions(topic_model, queries):
    ...
    logger.info(f"Generated topic distributions with shape: {topic_distributions.shape}")
...
def save_topic_results(question_ids, topics, topic_distributions, topic_info, 
                      topics_output_path, probabilities_output_path, info_output_path):
...
def validate_results(question_topics, topic_probabilities, topic_info):
    ...
    logger.info(f"Topic assignment distribution:")
    logger.info(f"  Most common topic: {topic_counts.index[0]} ({topic_counts.iloc[0]:,} questions)")
    logger.info(f"  Least common topic: {topic_counts.index[-1]} ({topic_counts.iloc[-1]:,} questions)")
    ...
    logger.info(f"Topic probability validation:")
    ...
    # Check topic info
    logger.info(f"Topic info validation:")
...
def main():
    ...
    try:
        ...
        questions_path = base_dir / "data/intermediate/question_analysis/english_questions.parquet"
        embeddings_path = base_dir / "data/intermediate/question_analysis/question_embeddings.parquet"
        topics_output_path = base_dir / "data/intermediate/question_analysis/question_topics.parquet"
        probabilities_output_path = base_dir / "data/intermediate/question_analysis/question_topic_probabilities.parquet"
        info_output_path = base_dir / "data/intermediate/question_analysis/topic_info.json"
    ...
    # Save results
    save_topic_results(
        question_ids, topics, topic_distributions, topic_info,
        topics_output_path, probabilities_output_path, info_output_path
    )
