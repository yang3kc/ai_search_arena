...
def create_summary_statistics_table(data):
    ...
    outcome_cols = [
        ...
        if any(pattern in col for pattern in [
            "proportion_left_leaning", "proportion_right_leaning", 
            "proportion_high_quality", "proportion_news", "num_citations"
        ])
    ]
    feature_cols = [
        col for col in data.columns
...
def create_correlation_matrix(data, max_vars=20):
    ...
    outcome_cols = [
        "proportion_left_leaning", "proportion_right_leaning", 
        "proportion_high_quality", "news_proportion_left_leaning",
...
def create_regression_coefficient_plots(results):
    ...
    for result in results['regression_results']:
        ...
        errors = [[c - l for c, l in zip(coefficients, conf_lower)],
...
def create_feature_importance_plot(results):
    ...
    avg_importance = {
        feat: np.mean(t_vals) 
        for feat, t_vals in feature_importance.items()
    }
    ...
    y_pos = range(len(feature_names))
    bars = ax.barh(y_pos, importance_scores, color=plt.cm.viridis(np.linspace(0, 1, len(importance_scores))))
    ...
    for i, (bar, score) in enumerate(zip(bars, importance_scores)):
        ax.text(score + max(importance_scores) * 0.01, i, f'{score:.2f}', 
...
def create_model_comparison_plots(data, results):
    ...
    model_cols = [col for col in data.columns if col.startswith("model_family_") and not col.endswith("_nan")]
...
def generate_html_report(data, results, output_path):
    ...
    html_content += f"""
        ...
        <div class="summary">
            ...
            <p><strong>PCA Applied:</strong> {results['metadata']['feature_engineering']['pca_applied']}</p>
            <p><strong>PCA Components:</strong> {results['metadata']['feature_engineering']['pca_components']}</p>
