...
def preprocess_questions(
    questions_df: pd.DataFrame, test_mode: bool = False
) -> pd.DataFrame:
    """
...
def create_embedding_dataframe(
    questions_df: pd.DataFrame, embeddings: np.ndarray, embedding_dim: int
) -> pd.DataFrame:
    """
    Create DataFrame with question ID and embeddings only.
    ...
    Returns:
        DataFrame with question_id and embedding columns only
    """
    logger.info("Creating embedding DataFrame...")

    # Create base DataFrame with only question_id
    embedding_df = questions_df[["question_id"]].copy()

    # Add embedding columns
    embedding_cols = [f"embedding_dim_{i}" for i in range(embedding_dim)]
    embedding_data = pd.DataFrame(
        embeddings, columns=embedding_cols, index=embedding_df.index
    )

    # Combine question_id and embeddings only
...
def main():
    ...
    embedding_df = create_embedding_dataframe(
        preprocessed_df, embeddings, generator.embedding_dim
    )
