    ...
    if "winner" in news_threads.columns:
        ...
        stats["winner_analysis"] = {
            ...
            "winner_distribution": {
                winner: {
                    "count": int(count),
                    "percentage": float(count / threads_with_winner * 100)
                    if threads_with_winner > 0
                    else 0,
                }
...
def analyze_question_patterns(questions, news_citations):
    ...
    if "turn_number" in news_questions.columns:
        ...
        stats["turn_analysis"] = {
            ...
            "first_turn_news_percentage": float(
                (news_questions["turn_number"] == 1).sum() / len(news_questions) * 100
            ),
        }
...
def analyze_model_comparison(data):
    ...
    stats["citations_by_model"] = {
        model: {"citations": int(count), "percentage": float(count / len(data) * 100)}
    ...
    if "response_id" in data.columns:
        model_response_stats = (
            data.groupby(model_col).agg({"response_id": ["nunique", "count"]}).round(2)
        )
...
def analyze_political_bias_patterns(data):
    ...
    if model_col:
        model_bias_stats = (
            bias_data.groupby(model_col)["political_leaning_score"]
    ...
    # Most biased domains
    domain_bias = (
        bias_data.groupby("domain")["political_leaning_score"]
...
def analyze_quality_patterns(data):
    ...
    stats["domain_quality_coverage"] = {
        ...
        "domain_coverage_percentage": float(
            domains_with_quality / unique_domains * 100
        ),
    }
    ...
    if model_col:
        model_quality_stats = (
            quality_data.groupby(model_col)["domain_quality_score"]
            .agg(["count", "mean", "median", "std"])
            .round(3)
        )
    ...
    domain_quality = (
        quality_data.groupby("domain")["domain_quality_score"]
        .agg(["count", "mean", "median"])
...
def analyze_joint_bias_quality(data):
    ...
    joint_data = data[
        data["political_leaning_score"].notna() & data["domain_quality_score"].notna()
    ].copy()
    ...
    stats["joint_coverage"] = {
        ...
        "domain_joint_coverage_percentage": float(
            domains_with_both / unique_domains * 100
        ),
    }

    # Correlation analysis
    correlation = joint_data["political_leaning_score"].corr(
        joint_data["domain_quality_score"]
    )
    ...
    quadrants = {
        "high_quality_left_leaning": len(
            joint_data[
                (joint_data["political_leaning_score"] < bias_median)
                & (joint_data["domain_quality_score"] > quality_median)
            ]
        ),
        "high_quality_right_leaning": len(
            joint_data[
                (joint_data["political_leaning_score"] > bias_median)
                & (joint_data["domain_quality_score"] > quality_median)
            ]
        ),
        "low_quality_left_leaning": len(
            joint_data[
                (joint_data["political_leaning_score"] < bias_median)
                & (joint_data["domain_quality_score"] < quality_median)
            ]
        ),
        "low_quality_right_leaning": len(
            joint_data[
                (joint_data["political_leaning_score"] > bias_median)
                & (joint_data["domain_quality_score"] < quality_median)
            ]
    ...
    stats["bias_quality_quadrants"] = {
        key: {"count": count, "percentage": float(count / len(joint_data) * 100)}
...
def generate_markdown_report(all_stats, output_path):
    ...
    if (
        "thread_patterns" in all_stats
        and "thread_overview" in all_stats["thread_patterns"]
    ):
    ...
    if (
        "question_patterns" in all_stats
        and "question_overview" in all_stats["question_patterns"]
    ):
    ...
    if (
        "response_patterns" in all_stats
        and "response_overview" in all_stats["response_patterns"]
    ):
    ...
    if "thread_patterns" in all_stats:
        ...
        report_lines.extend(["## News-Related Thread Analysis", ""])
        ...
        if "winner_analysis" in thread:
            ...
            if winner["winner_distribution"]:
                ...
                for side, stats in sorted(
                    winner["winner_distribution"].items(),
                    key=lambda x: x[1]["count"],
                    reverse=True,
                ):
                    report_lines.append(
                        f"  - {side}: {stats['count']:,} threads ({stats['percentage']:.1f}%)"
                    )
                report_lines.append("")
        ...
        if "intent_analysis" in thread:
            ...
            if intent["primary_intent_distribution"]:
                ...
                for intent_type, stats in sorted(
                    intent["primary_intent_distribution"].items(),
                    key=lambda x: x[1]["count"],
                    reverse=True,
                ):
                    report_lines.append(
                        f"  - {intent_type}: {stats['count']:,} threads ({stats['percentage']:.1f}%)"
                    )
                report_lines.append("")
    ...
    if "question_patterns" in all_stats:
        ...
        report_lines.extend(["## News-Related Question Analysis", ""])
        ...
        if "turn_analysis" in question:
            ...
            if turn["questions_by_turn"]:
                ...
                for turn_num, stats in sorted(turn["questions_by_turn"].items()):
                    report_lines.append(
                        f"  - Turn {turn_num}: {stats['count']:,} questions ({stats['percentage']:.1f}%)"
                    )
                report_lines.append("")
    ...
    if "response_patterns" in all_stats:
        ...
        report_lines.extend(["## News-Related Response Analysis", ""])
        ...
        if "model_side_analysis" in response:
            ...
            if side["responses_by_side"]:
                report_lines.extend(["### Model Side Distribution", ""])
                for side_name, stats in sorted(
                    side["responses_by_side"].items(),
                    key=lambda x: x[1]["count"],
                    reverse=True,
                ):
                    report_lines.append(
                        f"- **{side_name}**: {stats['count']:,} responses ({stats['percentage']:.1f}%)"
                    )
                report_lines.append("")
        ...
        if "citation_format_analysis" in response:
            ...
            if format_analysis["citations_by_format"]:
                report_lines.extend(["### Citation Format Distribution", ""])
                for format_type, stats in sorted(
                    format_analysis["citations_by_format"].items(),
                    key=lambda x: x[1]["count"],
                    reverse=True,
                ):
                    report_lines.append(
                        f"- **{format_type}**: {stats['count']:,} citations ({stats['percentage']:.1f}%)"
                    )
                report_lines.append("")
    ...
    if "news_relationships" in all_stats:
        ...
        report_lines.extend(["## News Citation Relationships", ""])
    ...
    if "domain_patterns" in all_stats:
        ...
        if "top_domains" in domain:
            ...
            for i, (domain_name, stats) in enumerate(
                list(top["top_20_domains"].items())[:10], 1
            ):
                report_lines.append(
                    f"{i}. **{domain_name}**: {stats['citations']:,} citations ({stats['percentage']:.1f}%)"
                )
            report_lines.append("")
    ...
    if "model_comparison" in all_stats:
        ...
        for model_name, stats in sorted_models[:10]:
            report_lines.append(
                f"- **{model_name}**: {stats['citations']:,} citations ({stats['percentage']:.1f}%)"
            )
        report_lines.append("")
    ...
    if "political_bias" in all_stats:
        ...
        if "political_leaning_distribution" in bias:
            ...
            report_lines.extend(["### Political Leaning Distribution", ""])

            for leaning, stats in sorted(
                leaning_dist.items(), key=lambda x: x[1]["count"], reverse=True
            ):
                report_lines.append(
                    f"- **{leaning}**: {stats['count']:,} citations ({stats['percentage']:.1f}%)"
                )
            report_lines.append("")
...
def main():
    ...
    all_stats = {
        ...
        "question_patterns": analyze_question_patterns(
            all_data["questions"], news_citations
        ),
        "response_patterns": analyze_response_patterns(
            all_data["responses"], news_citations
        ),
    ...
    if "domain_patterns" in all_stats:
        logger.info(
            f"Unique domains: {all_stats['domain_patterns']['domain_overview']['unique_domains']:,}"
        )
    if (
        "political_bias" in all_stats
        and "bias_score_distribution" in all_stats["political_bias"]
    ):
        bias_coverage = all_stats["political_bias"]["bias_score_distribution"][
            "coverage_percentage"
        ]
        logger.info(f"Political bias coverage: {bias_coverage:.1f}%")
    if (
        "quality_patterns" in all_stats
        and "quality_score_distribution" in all_stats["quality_patterns"]
    ):
        quality_coverage = all_stats["quality_patterns"]["quality_score_distribution"][
            "coverage_percentage"
        ]
