...
def integrate_citation_data(citations, responses, threads, questions):
    ...
    if "secondary_intent" not in responses.columns:
        print(
            "  Warning: secondary_intent not found in responses, will get from threads"
        )

    integrated = integrated.merge(
        responses[available_response_cols], on="response_id", how="left"
    )
    ...
    integrated = integrated.merge(
        threads[thread_cols_to_merge], on="thread_id", how="left"
    )
    ...
    if "question_text" in questions.columns and "question_id" in integrated.columns:
        integrated = integrated.merge(
            questions[question_cols], on="question_id", how="left"
        )
        print(f"  After joining with questions: {len(integrated):,} citations")
    else:
        print(
            "  Skipping question text join (question_id or question_text not available)"
        )
...
def add_derived_features(integrated):
    ...
    if "winner" in integrated.columns and "model_side" in integrated.columns:
        ...
        integrated["model_won"] = (integrated["winner"] == "model_a") & (
            integrated["model_side"] == "a"
        ) | (integrated["winner"] == "model_b") & (integrated["model_side"] == "b")
        integrated["model_lost"] = (integrated["winner"] == "model_a") & (
            integrated["model_side"] == "b"
        ) | (integrated["winner"] == "model_b") & (integrated["model_side"] == "a")
        integrated["model_tied"] = integrated["winner"].isin(
            ["tie", "tie (bothbad)", ""]
        )
    ...
    if "model_name_llm" in integrated.columns:
        ...
        integrated["model_family"] = integrated["model_name_llm"].apply(
            get_model_family
        )
