...
def analyze_overall_bias_distribution(data):
    ...
    left_domains = bias_data.nsmallest(10, 'political_leaning_score')[['domain', 'political_leaning_score']].drop_duplicates('domain')
    for _, row in left_domains.head(5).iterrows():
        citations = len(bias_data[bias_data['domain'] == row['domain']])
        print(f"  {row['domain']}: {row['political_leaning_score']:.3f} ({citations} citations)")
    
    print(f"\nMost right-leaning domains (highest scores):")
    right_domains = bias_data.nlargest(10, 'political_leaning_score')[['domain', 'political_leaning_score']].drop_duplicates('domain')
...
def analyze_model_bias_patterns(data):
    ...
    model_bias_stats = bias_data.groupby('model_name_raw')['political_leaning_score'].agg([
    ...
    for model, stats in model_bias_stats.iterrows():
        print(f"{model:<35} {stats['mean']:<8.3f} {stats['median']:<8.3f} {stats['std']:<8.3f} {stats['count']:<8.0f}")
    ...
    if 'model_family' in bias_data.columns:
        ...
        for family, stats in family_bias_stats.iterrows():
            print(f"{family:<15} {stats['mean']:<8.3f} {stats['median']:<8.3f} {stats['std']:<8.3f} {stats['count']:<8.0f}")
    
    # Political leaning distribution by model
    model_leaning_crosstab = pd.crosstab(bias_data['model_name_raw'], bias_data['political_leaning'])
    model_leaning_pcts = model_leaning_crosstab.div(model_leaning_crosstab.sum(axis=1), axis=0) * 100
    ...
    if len(models) >= 2:
        ...
        for i, model1 in enumerate(models):
            for model2 in models[i+1:]:
                scores1 = bias_data[bias_data['model_name_raw'] == model1]['political_leaning_score']
                scores2 = bias_data[bias_data['model_name_raw'] == model2]['political_leaning_score']
        ...
        for model1, model2, p_val, mean_diff in sorted(significant_pairs, key=lambda x: x[2])[:5]:
            direction = "more left" if mean_diff < 0 else "more right"
            print(f"  {model1} vs {model2}: p={p_val:.3e} ({model1} {direction} by {abs(mean_diff):.3f})")
    
    return {
        'model_bias_stats': model_bias_stats.to_dict(),
        'family_bias_stats': family_bias_stats.to_dict() if 'model_family' in bias_data.columns else {},
        'model_leaning_percentages': model_leaning_pcts.to_dict(),
        'significant_differences': significant_pairs if 'significant_pairs' in locals() else []
    }
...
def analyze_intent_bias_patterns(data):
    ...
    intent_bias_stats = bias_data.groupby('primary_intent')['political_leaning_score'].agg([
        'mean', 'median', 'std', 'count'
    ]).round(3)
    ...
    for intent, stats in intent_bias_stats.iterrows():
        print(f"{intent:<20} {stats['mean']:<8.3f} {stats['median']:<8.3f} {stats['std']:<8.3f} {stats['count']:<8.0f}")
    
    # Political leaning distribution by intent
    intent_leaning_crosstab = pd.crosstab(bias_data['primary_intent'], bias_data['political_leaning'])
    intent_leaning_pcts = intent_leaning_crosstab.div(intent_leaning_crosstab.sum(axis=1), axis=0) * 100
...
def analyze_winner_bias_patterns(data):
    ...
    print("-" * 51)
    print(f"{'Mean':<15} {winner_stats['mean']:<12.3f} {loser_stats['mean']:<12.3f} {winner_stats['mean'] - loser_stats['mean']:<12.3f}")
    print(f"{'Median':<15} {winner_stats['50%']:<12.3f} {loser_stats['50%']:<12.3f} {winner_stats['50%'] - loser_stats['50%']:<12.3f}")
    print(f"{'Std':<15} {winner_stats['std']:<12.3f} {loser_stats['std']:<12.3f} {winner_stats['std'] - loser_stats['std']:<12.3f}")
    ...
    print(f"\nPolitical leaning distribution (%):")
    comparison_df = pd.DataFrame({'Winners': winner_leaning, 'Losers': loser_leaning}).fillna(0)
    comparison_df['Difference'] = comparison_df['Winners'] - comparison_df['Losers']
    print(comparison_df.round(1))
    
    # Statistical significance test
    _, p_value = mannwhitneyu(winners['political_leaning_score'], 
...
def analyze_bias_over_time(data):
    ...
    # Daily bias trends
    daily_bias = bias_data.groupby('date')['political_leaning_score'].agg(['mean', 'count']).reset_index()
    daily_bias = daily_bias[daily_bias['count'] >= 10]  # Filter days with sufficient data
    ...
    # Weekly trends
    weekly_bias = bias_data.groupby('week')['political_leaning_score'].agg(['mean', 'count']).reset_index()
    weekly_bias = weekly_bias[weekly_bias['count'] >= 50]  # Filter weeks with sufficient data
    
    if len(weekly_bias) > 1:
        # Trend analysis
        week_numbers = range(len(weekly_bias))
        slope, intercept, r_value, p_value, std_err = linregress(week_numbers, weekly_bias['mean'])
...
def create_bias_visualizations(analysis_results, data, output_dir):
    ...
    ax.hist(bias_data['political_leaning_score'], bins=50, alpha=0.7, density=True, 
            color='lightblue', edgecolor='black')
    
    # Add vertical lines for categories
    ax.axvline(x=0, color='gray', linestyle='--', alpha=0.7, label='Center')
    ax.axvline(x=bias_data['political_leaning_score'].mean(), color='red', 
               linestyle='-', alpha=0.8, label=f'Mean ({bias_data["political_leaning_score"].mean():.3f})')
    
    ax.set_title('Distribution of Political Bias Scores in News Citations', fontsize=16, pad=20)
    ax.set_xlabel('Political Leaning Score (Negative = Left, Positive = Right)', fontsize=12)
    ...
    if 'model_leaning_percentages' in analysis_results:
        ...
        sns.heatmap(model_leaning_df, annot=True, fmt='.1f', cmap='RdBu_r', 
                   center=33.33, ax=ax, cbar_kws={'label': '% of Citations'})
    ...
    if 'intent_bias_stats' in analysis_results and 'primary_intent' in bias_data.columns:
        fig, ax = plt.subplots(figsize=(12, 8))
        
        intent_means = bias_data.groupby('primary_intent')['political_leaning_score'].mean().sort_values()
        intent_counts = bias_data.groupby('primary_intent').size()
        
        # Bar plot with error bars
        bars = ax.bar(range(len(intent_means)), intent_means.values, 
                     color=['red' if x > 0 else 'blue' for x in intent_means.values], alpha=0.7)
    ...
    if 'winner_bias_stats' in analysis_results:
        ...
        # Box plot comparison
        winner_data = bias_data[bias_data['model_won'] == True]['political_leaning_score']
        loser_data = bias_data[bias_data['model_lost'] == True]['political_leaning_score']
        ...
        bars = ax2.bar(['Winners', 'Losers'], means, yerr=stds, 
        ...
        if 'statistical_test' in analysis_results:
            ...
            ax2.text(0.5, max(means) + max(stds) * 0.1, f'p={p_val:.3e} {significance}', 
...
def generate_bias_report(analysis_results, data, output_path):
    ...
    summary_stats = pd.DataFrame({
        ...
        'left_leaning_pct': [len(bias_data[bias_data['political_leaning'] == 'left_leaning']) / len(bias_data) * 100],
        'right_leaning_pct': [len(bias_data[bias_data['political_leaning'] == 'right_leaning']) / len(bias_data) * 100]
    })
