...
def load_coefficients_data(input_files):
    ...
    if input_files['individual_effects_coefficients'].exists():
        coefficients['individual_effects'] = pd.read_csv(input_files['individual_effects_coefficients'])
        print(f"  Loaded individual coefficients: {len(coefficients['individual_effects'])} rows")
    
    # Load citation style effects coefficients
    if input_files['citation_style_effects_coefficients'].exists():
        coefficients['citation_style_effects'] = pd.read_csv(input_files['citation_style_effects_coefficients'])
        print(f"  Loaded citation style coefficients: {len(coefficients['citation_style_effects'])} rows")
...
def create_individual_effects_plot(coefficients, output_path):
    ...
    if not plot_data:
        ...
        plt.text(0.5, 0.5, 'No individual effects data available', 
        ...
        plt.errorbar(df['coefficient'], y_pos, 
                    xerr=[df['coefficient'] - df['ci_lower'], df['ci_upper'] - df['coefficient']],
                    fmt='o', capsize=5, capthick=2, color='black', alpha=0.7)
        ...
        plt.title('Individual Feature Effects - 95% Confidence Intervals\n(Red = Significant, Blue = Not Significant)', fontsize=14)
...
def create_citation_style_effects_plot(coefficients, output_path):
    ...
    if 'citation_style_effects' in coefficients:
        ...
        model_groups = coefficients['citation_style_effects'].groupby('model_specification')
        ...
        for model_name, model_data in model_groups:
            ...
            if plot_data:
                ...
                # Plot confidence intervals as error bars
                ax.errorbar(df['coefficient'], y_pos, 
                           xerr=[df['coefficient'] - df['ci_lower'], df['ci_upper'] - df['coefficient']],
                           fmt='o', capsize=4, capthick=1.5, color='black', alpha=0.7)
        ...
        for ax in axes:
            ax.text(0.5, 0.5, 'No citation style effects data available', 
                   ha='center', va='center', transform=ax.transAxes, fontsize=14)
            ax.set_title('Citation Style Effects - No Data Available')
    
    plt.suptitle('Citation Style Effects - 95% Confidence Intervals by Model\n(Red = Significant, Blue = Not Significant)', 
                 fontsize=16, y=0.98)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if 'bt_ratings' in coefficients:
        ...
        if len(bt_data) > 0:
            ...
            for i, rating in enumerate(bt_data['coefficient']):
                plt.text(rating + max(bt_data['coefficient']) * 0.01, i, f'{rating:.0f}', 
                        va='center', fontsize=10)
        else:
            plt.text(0.5, 0.5, 'No Bradley-Terry rating data available', 
                    ha='center', va='center', transform=plt.gca().transAxes, fontsize=14)
...
def generate_overview_section(results, battle_df):
    ...
    n_models = len(set(battle_df['model_a']) | set(battle_df['model_b'])) if battle_df is not None else 0
...
"""
    ...
    sorted_models = sorted(bt_results['model_ratings'].items(), key=lambda x: x[1], reverse=True)
    ...
    try:
        ...
        if 'snakemake' in globals():
            input_files = {
                ...
                'individual_effects_results': Path(snakemake.input.individual_effects_results),
                'citation_style_effects_results': Path(snakemake.input.citation_style_effects_results),
                'bt_ratings_coefficients': Path(snakemake.input.bt_ratings_coefficients),
                'individual_effects_coefficients': Path(snakemake.input.individual_effects_coefficients),
                'citation_style_effects_coefficients': Path(snakemake.input.citation_style_effects_coefficients),
        ...
        create_citation_style_effects_plot(coefficients, output_files['citation_style_effects'])
        create_model_comparison_plot(coefficients, output_files['model_comparison'])
        
        # Generate comprehensive HTML report
        generate_html_report(results, coefficients, input_files['battle_data'], output_files['report'])
