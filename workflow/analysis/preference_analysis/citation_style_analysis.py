...
class CitationStyleAnalyzer:
    ...
    def __init__(
        self, anchor_model: str = "gpt-4o-search-preview", anchor_rating: float = 1000.0
    ):
        self.anchor_model = anchor_model
        self.anchor_rating = anchor_rating

    def load_data(
        self, threads_path: str, responses_path: str, citations_path: str
    ...
    def compute_citation_features(self, citations_df: pd.DataFrame) -> pd.DataFrame:
        ...
        basic_counts = (
            citations_with_categories.groupby("response_id")
            .agg({"citation_id": "count"})
            .rename(columns={"citation_id": "num_citations"})
        )

        # Domain-specific counts
        domain_counts = (
            citations_with_categories.groupby(["response_id", "domain_category"])
            .size()
        ...
        for domain in domain_categories:
            features_df[f"has_{domain}"] = (features_df[f"cites_{domain}"] > 0).astype(
                int
            )
    ...
    def _categorize_domains(self, citations_df: pd.DataFrame) -> pd.DataFrame:
        ...
        def categorize_domain(domain):
            ...
            if any(
                map_term in domain
                for map_term in ["maps.google", "openstreetmap", "mapquest"]
            ):
                return "map"
        ...
        citations_df["domain_category"] = citations_df["domain"].apply(
            categorize_domain
        )
        return citations_df
    ...
    def create_battle_data(
        ...
        citation_features_df: pd.DataFrame,
    ) -> pd.DataFrame:
        """
        ...
        for _, thread in threads_df.iterrows():
            ...
            for question_id in thread_responses["question_id"].unique():
                ...
                feature_cols = [
                    col for col in citation_features_df.columns if col != "response_id"
                ]
    ...
    def compute_style_coefficients(
        self,
        ...
        battle_clean = battle_df[
            ~battle_df["winner"].isin(["tie", "tie (bothbad)"])
        ].copy()
        ...
        for feature in style_features:
            if (
                f"{feature}_a" in battle_clean.columns
                and f"{feature}_b" in battle_clean.columns
            ):
        ...
        coefficients, log_likelihood = self._fit_style_model(
            battle_clean, style_columns
        )
        ...
        for _ in range(num_bootstrap):
            try:
                # Resample battles
                bootstrap_sample = battle_clean.sample(
                    n=len(battle_clean), replace=True
                )
    ...
    def _fit_style_model(
        self, battle_df: pd.DataFrame, style_columns: List[str]
        ...
        n_style_features = (
            len(style_columns) // 2
        )  # Divide by 2 since we have _a and _b versions
        ...
        if not np.isfinite(initial_ll):
            warnings.warn(
                "Initial log-likelihood is not finite - may indicate data issues"
            )
        ...
        for method in methods_to_try:
            try:
                if method == "L-BFGS-B":
                    ...
                    result = opt.minimize(
                        neg_log_likelihood, initial_params, method=method, bounds=bounds
                    )
                else:
                    result = opt.minimize(
                        neg_log_likelihood, initial_params, method=method
                    )
...
if __name__ == "__main__":
    ...
    domain_features = [
        ...
        "cites_foreign_news",
        "cites_social_media",
        "cites_community_blog",
        "cites_tech_coding",
        "cites_academic_journal",
    ]
